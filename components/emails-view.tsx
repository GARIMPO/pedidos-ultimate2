...
import { Mail, Trash2, Search, Phone } from "lucide-react"
...
export default function EmailsView({ contatos, onDelete }: EmailsViewProps) {
  ...
  const abrirWhatsApp = (telefone: string, nome: string) => {
    ...
    const mensagem = encodeURIComponent(`Olá ${nome}, seu pedido já foi enviado`)
                      ...
                      <div className="flex items-center justify-between">
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-1 text-muted-foreground">
                            ...
                            <a
                              href={`https://wa.me/${contato.telefone?.replace(/\D/g, "")}?text=${encodeURIComponent(`Olá ${contato.nome}, seu pedido já foi enviado`)}`}
                              className="hover:text-primary cursor-pointer"
                              onClick={(e) => {
                                e.preventDefault()
                                if (contato.telefone) {
